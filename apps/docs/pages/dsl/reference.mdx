...
<Callout type="info">
<strong>Complete API reference.</strong> Every method on EpisodeBuilder, DeviceBuilder, BeatBuilder, and CameraBuilder documented here.
</Callout>
...
b.typingStart("Bob");                 // Start typing (manual end)
b.typingEnd("Bob");                   // End typing
```
...
b.read(msg);                          // Mark as read
b.readLast();                         // Mark last message as read
b.delete(msg);                        // Delete message
b.deleteLast();                       // Delete last message
b.react(msg, "me", "❤️");             // Add reaction
```
...
b.goBack();                           // Go back
b.navigate("app_twitter");            // Switch to different app
b.notification("app_whatsapp", {      // Show notification
    title: "New Message",
    body: "Hey!",
    mode: "headsup"
});
...
```typescript
b.pov("BobPhone");                    // Switch to device
b.pov("BobPhone", "crossfade");       // With transition
...
b.missedCall("Bob");                   // Missed call
b.missedCall("Bob", "video");         // Missed video call
```
...
```typescript
// Post a tweet
b.postTweet("Hello world!", {
    author: { name: "You", handle: "you", verified: "blue" }
});

// Receive a tweet
const tweet = b.tweetReceived(
    { name: "Elon", handle: "elonmusk", verified: "blue" },
    "The future is here 🚀",
    { likeCount: 89000, retweetCount: 12800 }
);

// Engagement
b.likeTweet(tweet);
b.retweetTweet(tweet);
b.quoteTweet(tweet, "This is huge!");
b.replyTweet(tweet, "Agreed!");
b.bookmarkTweet(tweet);
```

### Keyboard Simulation

```typescript
b.showKeyboard();                     // Show keyboard
b.showKeyboard("emoji");              // Emoji keyboard
b.hideKeyboard();                     // Hide keyboard
b.simulateTyping("Hello!", {          // Realistic typing
    speed: "casual",                  // slow|casual|fast|angry
    typos: 2                          // Optional typos
});
```
