...
import { getPostizApiKey, getPostizBaseUrl } from './config'
...
function normalizeIntegration(record: PostizIntegrationRecord): PublishingChannel | null {
  ...
  return publishingChannelSchema.parse({
    ...
    status:
      record.connected === false || record.status === 'disconnected' ? 'disconnected' : 'connected',
...
export class PostizPublisher implements SocialPublisher {
  async listChannels(): Promise<PublishingChannel[]> {
    ...
    const body = (await response.json()) as
      ...
      | { items?: PostizIntegrationRecord[]; integrations?: PostizIntegrationRecord[]; data?: PostizIntegrationRecord[] }

    const records = Array.isArray(body)
      ? body
      : body.items ?? body.integrations ?? body.data ?? []

    return records.map(normalizeIntegration).filter((value): value is PublishingChannel => value !== null)
  }
