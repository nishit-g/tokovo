...
function getFirstEnvValue(keys, env = getResolvedEnv()) {
...
export function getRenderServicePollMs() {
...
export function getRenderMaxConcurrency() {
...
function getRawR2Config(env = getResolvedEnv()) {
    return {
        bucket: getFirstEnvValue(["TOKOVO_R2_BUCKET", "R2_BUCKET"], env),
        region: getFirstEnvValue(["TOKOVO_R2_REGION", "R2_REGION"], env) ?? "auto",
        endpoint: getFirstEnvValue(["TOKOVO_R2_ENDPOINT", "R2_ENDPOINT"], env) ??
            (() => {
                const accountId = getFirstEnvValue(["TOKOVO_R2_ACCOUNT_ID", "R2_ACCOUNT_ID"], env);
                return accountId
                    ? `https://${accountId}.r2.cloudflarestorage.com`
                    : undefined;
            })(),
        accessKeyId: getFirstEnvValue(["TOKOVO_R2_ACCESS_KEY_ID", "R2_ACCESS_KEY_ID"], env),
        secretAccessKey: getFirstEnvValue(["TOKOVO_R2_SECRET_ACCESS_KEY", "R2_SECRET_ACCESS_KEY"], env),
        publicBaseUrl: getFirstEnvValue(["TOKOVO_R2_PUBLIC_BASE_URL", "R2_PUBLIC_BASE_URL"], env),
        keyPrefix: getFirstEnvValue(["TOKOVO_R2_KEY_PREFIX", "R2_KEY_PREFIX"], env) ?? "",
    };
}
export function hasAnyR2Config() {
    const raw = getRawR2Config();
    return Boolean(raw.bucket ||
        raw.endpoint ||
        raw.accessKeyId ||
        raw.secretAccessKey ||
        raw.publicBaseUrl ||
...
export function getR2ConfigValidationError() {
    const raw = getRawR2Config();
    ...
    if (!raw.bucket)
        missing.push("bucket");
    if (!raw.endpoint)
        missing.push("endpoint (or account id)");
    if (!raw.accessKeyId)
