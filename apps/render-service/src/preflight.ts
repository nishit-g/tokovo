...
export type RenderDoctorCheck = {
  id: string
  ok: boolean
  message: string
}

export type RenderDoctorResult = {
  ok: boolean
  checks: RenderDoctorCheck[]
}
...
async function canWriteOutputDirectory(): Promise<boolean> {
...
export async function runRenderDoctor(): Promise<RenderDoctorResult> {
  const checks: RenderDoctorCheck[] = []
...
export async function assertRenderPreflight(): Promise<void> {
