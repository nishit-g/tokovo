...
export function getResolvedEnv(): EnvLike {
  ...
  for (const filePath of [
    path.join(repoRoot, '.env.payload.local'),
...
function getFirstEnvValue(keys: string[], env: EnvLike = getResolvedEnv()): string | undefined {
...
export function getPublicAssetBaseUrl(): string | null {
  const raw = getFirstEnvValue(['TOKOVO_PUBLIC_ASSET_BASE_URL', 'TOKOVO_ASSET_BASE_URL'])
...
export function getR2Config(): R2Config | null {
  ...
  const raw = getRawR2Config()
  if (!hasAnyR2Config() || !raw.bucket || !raw.endpoint || !raw.accessKeyId || !raw.secretAccessKey) {
    return null
  }
