...
device-pixel-ratio hint. Phone apps use `display.width / pointScale` as their logical width, so
app navigation, status bars, keyboards, and notifications share one point grid. App layouts use
their point-sized tokens directly; they must not rescale them relative to a fixed 393-point canvas.
`AppSurface` maps that logical width onto the physical display. Non-phone surfaces retain their
...
Opaque app navigation surfaces should not add decorative backdrop blur: it can sample across
rounded display edges during compositing and make the bezel look detached. The idle Dynamic Island
is opaque hardware, not an elevated card; active activity surfaces retain their platform material.
...
The current architecture replaced:

- shared iOS/Android typography metrics;
- app-owned safe-area defaults;
- app-owned keyboard and notification themes;
- empty keyboard suggestion strips;
- approximate shared Devanagari layouts;
- flat notification cards and unstructured center blur;
- screen content mounted flush against the outer device frame;
- emoji-based system and app empty-state placeholders;
- always-expanded recording activity;
- generic renderer layouts and missing-device substitutions;
- broad ungoverned background presets;
- cardinality-dependent app state;
- overlay state disguised as app state.

Regression proof covers iOS and Android keyboards, Arabic/RTL and Devanagari composition,
notification grouping/privacy/materials, lock/home surfaces, compact/expanded/stopped recording,
physical screen inset, multi-device editorial composition, and approved golden frames. Goldens are
accepted only after geometry and product review.
