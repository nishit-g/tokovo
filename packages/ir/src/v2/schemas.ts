...
export const OSConfigSchema = z.object({
  ...
  textScale: z.number().min(0.75).max(2).optional(),
  contrast: z.enum(["standard", "increased"]).optional(),
  motion: z.enum(["full", "reduced"]).optional(),
  transparency: z.enum(["standard", "reduced"]).optional(),
  materialPreference: z.enum(["automatic", "regular", "clear"]).optional(),
  colorSeed: z.string().min(1).optional(),
