...
function estimatedLineCount(text: string, width: number, fontSize: number, maxLines: number): number {
  if (!text.trim()) return 0;
  const charactersPerLine = Math.max(8, Math.floor(width / (fontSize * 0.56)));
  const paragraphs = text.split("\n");
  const lines = paragraphs.reduce(
    (total, paragraph) => total + Math.max(1, Math.ceil(Array.from(paragraph).length / charactersPerLine)),
    0,
  );
  return Math.min(maxLines, lines);
}

function attachmentHeight(tweet: XTweet, contentWidth: number, detail: boolean): number {
  if (tweet.media) {
    if (tweet.media.aspect === "square") return Math.min(contentWidth, detail ? 340 : 280);
    if (tweet.media.aspect === "tall") return Math.min(contentWidth * 1.18, detail ? 420 : 330);
...
export function measureXPost(tweet: XTweet, viewportWidth: number, detail = false): XPostMeasurement {
  ...
  const bodyLines = estimatedLineCount(tweet.text, contentWidth, fontSize, detail ? 12 : 8);
  ...
  return {
    totalHeight: repostLabelHeight + headerHeight + bodyHeight + mediaHeight + metricsHeight + gaps + verticalPadding,
    repostLabelHeight,
    headerHeight,
    bodyHeight,
...
export function measureXMessage(text: string, viewportWidth: number): XMessageMeasurement {
  const bubbleWidth = Math.min(Math.round(viewportWidth * 0.76), 292);
  const lines = estimatedLineCount(text, bubbleWidth - 28, 15, 20);
  return {
    height: Math.max(38, lines * 20 + 20),
    lines,
    bubbleWidth,
  };
}
