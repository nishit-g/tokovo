...
export function computeXFullscreenLayout(ctx: LayoutContext): FullscreenLayoutState {
  ...
  region(
    ...
    "x.composer.editor",
    rect(
      16 + experience.text.avatar + experience.text.avatarGap,
      editorY,
      width - 32 - experience.text.avatar - experience.text.avatarGap,
      Math.max(0, audienceY - editorY),
    ),
    ["compose", "editor"],
  );
  region(regions, "x.composer.audience", rect(16, audienceY, width - 32, audienceHeight), [
    "compose",
    "audience",
  ]);
  region(
    regions,
    "x.composer.actions",
    rect(0, actionsY, width, actionsHeight),
    ["compose", "actions", "sticky"],
    { sticky: true },
  );
  return {
    kind: "FULLSCREEN",
    ...
    meta: {
      draftLength: Array.from(state.composer.draft).length,
