...
describe("X VNext experience resolver", () => {
  it("resolves platform-specific typography and touch metrics", () => {
    ...
    const android = resolveXExperience({
      platform: "android",
      appearance: "light",
      locale: "en-US",
      reducedMotion: false,
    ...
    expect(ios.type.family).toContain("Inter Variable");
    expect(android.type.family).toContain("Roboto Variable");
  ...
  it("resolves light, dim, and lights-out as distinct palettes", () => {
    const light = resolveXExperience({
      platform: "ios",
      appearance: "light",
      locale: "en-US",
      reducedMotion: false,
    ...
    const lightsOut = resolveXExperience({
      platform: "ios",
      appearance: "dark",
  ...
  it("rejects unsupported themes and light/dark mismatches", () => {
    ...
    expect(() => resolveXExperience({ ...base, themeId: "storybook" })).toThrow(
      /X_THEME_UNSUPPORTED/,
    );
    expect(() => resolveXExperience({ ...base, themeId: "x-dim" })).toThrow(
      /X_THEME_APPEARANCE_MISMATCH/,
    );
  });
});
