export const xAnchor = {
  ...
  post: (tweetId: string, region: "card" | "author" | "body" | "media" | "poll" | "quote" | "link" | "metrics" = "card") =>
    region === "card" ? `x.post.${tweetId}` : `x.post.${tweetId}.${region}`,
  notification: (notificationId: string) => `x.notification.${notificationId}`,
  profile: (userId: string, region: "header" | "banner" | "avatar" = "header") =>
    `x.profile.${userId}.${region}`,
  thread: (threadId: string) => `x.dm.${threadId}`,
  message: (threadId: string, messageId: string) => `x.dm.${threadId}.message.${messageId}`,
  composer: {
