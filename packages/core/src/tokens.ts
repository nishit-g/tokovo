...
const typographyMap: Record<
  SemanticTypography,
  {
    ios: keyof typeof iOSTokens.typography;
    android: keyof typeof androidTokens.typography;
  }
> = {
...
export function getAppConfig<T extends keyof typeof appConfigs>(
  ...
  const config = appConfigs[app] as Record<
    Platform,
    (typeof appConfigs)[T][Platform]
  >;
  return config[platform] || config.ios;
}
