...
export const BatteryIcon: React.FC<
  ScaledIconProps & { percentage?: number; charging?: boolean }
> = ({ color = "currentColor", percentage = 100, charging = false, scale = 1 }) => (
  <svg width={25 * scale} height={12 * scale} viewBox="0 0 25 12" fill="none">
    ...
    {/* Charging bolt */}
    {charging && <path d="M11 2L8 6H11L10 10L13 6H10L11 2Z" fill="white" />}
  </svg>
);
...
export const NetworkTypeLabel: React.FC<{
  network: string;
  color: string;
  fontSize?: number;
  fontFamily: string;
}> = ({ network, color, fontSize = 12, fontFamily }) => {
