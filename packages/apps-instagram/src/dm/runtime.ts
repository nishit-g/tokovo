...
export function dmReducer(draft: WorldState, event: TimelineEvent, instagram: InstagramState) {
    ...
    const ensureConversation = (id: string) => {
        // Initialize dm state if missing
        if (!instagram.dm) instagram.dm = { conversations: {} };
        if (!instagram.dm.conversations) instagram.dm.conversations = {};

        if (!instagram.dm.conversations[id]) {
            instagram.dm.conversations[id] = { id, messages: [] };
        }
        return instagram.dm.conversations[id];
    };
