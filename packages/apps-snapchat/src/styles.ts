...
export function typingDotStyle(index: number, frame: number, fps: number): CSSProperties {
  const progress = pulse(frame, fps, 1.4, index * 0.16);
  return {
    ...
    transform: `translateY(${-6 * progress}px)`,
  };
}
...
export function indicatorShapeStyle(type: "snap" | "chat", opened: boolean): CSSProperties {
