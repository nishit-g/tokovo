...
const VideoIcon: React.FC<{ size?: number; color?: string }> = ({
  size = 20,
  color = snapchatColors.textSecondary,
}) => (
  <svg
...
const SmileyIcon: React.FC<{ size?: number; color?: string }> = ({
  size = 20,
  color = snapchatColors.textSecondary,
}) => (
  <svg
...
const ComposeIcon: React.FC<{ size?: number; color?: string }> = ({
  size = 22,
  color = snapchatColors.textSecondary,
}) => (
  <svg
...
const TypingDots: React.FC = () => {
  ...
  return (
    <div style={typingIndicatorContainerStyle()}>
      ...
      >
        <div style={typingDotStyle(0, frame, fps)} />
        <div style={typingDotStyle(1, frame, fps)} />
        <div style={typingDotStyle(2, frame, fps)} />
      </div>
    </div>
  );
};
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "hidden" }}>
        {conversations.map((conv) => (
          <ChatListRow key={conv.id} conversation={conv} world={world} deviceId={deviceId} />
        ))}
      </div>

      {/* Bottom nav */}
      <div style={{ paddingBottom: contentBottom }}>
...
}> = ({ state, contentTop, contentBottom, keyboardHeight }) => {
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        paddingTop: contentTop,
        paddingBottom: keyboardHeight > 0 ? 0 : contentBottom,
        boxSizing: "border-box",
      }}
    >
...
const MessageRow: React.FC<{ message: SnapchatMessage }> = ({ message }) => {
  ...
  if (message.kind === "snap") {
    ...
    const color = isReceived
      ? opened
        ? snapchatColors.indicatorGray
        : snapchatColors.indicatorRed
      : opened
        ? snapchatColors.indicatorGray
        : snapchatColors.arrowSent;
    const label = isReceived
      ? opened
        ? "Opened"
        : message.snapType === "video"
          ? "New Video"
          : "New Snap"
      : opened
        ? "Opened"
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "linear-gradient(180deg, rgba(0,0,0,0.42) 0%, rgba(0,0,0,0.08) 32%, rgba(0,0,0,0.48) 100%)",
        }}
      />
      <div
        style={{
    ...
    case "chat":
      return (
        <ChatScreen
          ...
          contentBottom={contentInsets.bottom}
