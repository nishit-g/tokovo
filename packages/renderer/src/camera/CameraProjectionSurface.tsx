...
function displacementVector(input: {
  ...
  if (input.kind === "edge-horizontal" || input.kind === "edge-vertical") {
    const axisValue =
      input.kind === "edge-horizontal" ? normalizedX : normalizedY;
    ...
    return input.kind === "edge-horizontal"
      ? [displacement, 0]
      : [0, displacement];
  }
...
function bytesToBase64(bytes: Uint8Array): string {
  ...
  for (let index = 0; index < bytes.length; index += 3) {
    ...
    result +=
      index + 1 < bytes.length ? BASE64_ALPHABET[(packed >>> 6) & 63] : "=";
...
export function createDisplacementMapDataUri(input: {
  ...
  const pixels = new Uint8Array(
    DISPLACEMENT_MAP_SIZE * DISPLACEMENT_MAP_SIZE * 4,
  );
...
function projectiveTransform(
  ...
  const projective = passes.find(
    (
      pass,
    ): pass is Extract<CameraProjectionPass, { kind: "projective-warp" }> =>
  ...
  for (const pass of props.passes) {
    ...
    if (pass.kind === "directional-smear" && pass.spreadPx > 0) {
      ...
      const directionLength = Math.max(
        1e-6,
        Math.hypot(pass.direction[0], pass.direction[1]),
      );
 ...
 * cinematic subjects.
 */
export const CameraProjectionSurface: React.FC<
  CameraProjectionSurfaceProps
> = ({ id, output, stageWidth, stageHeight, children }) => {
  ...
  const hasFilter = output.projectionPasses.some(
    (pass) => pass.kind !== "projective-warp",
  );
          ...
          >
            <div
              style={{
                ...
                height: stageHeight,
                transformOrigin: "0 0",
                transform: matrix3ToCssMatrix(output.viewMatrix, [
                  viewport.x,
                  viewport.y,
                ]),
              }}
