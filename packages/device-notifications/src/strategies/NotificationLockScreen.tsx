...
export const NotificationLockScreen: React.FC<NotificationLockScreenProps> = ({
  notifications,
  tokens,
  scale,
  fps = 30,
  ...
  const getTransformForItem = (
    ...
    if (animationState === "entering") {
      const translateY =
        animation.lockScreenEnterTranslateY * (1 - animationProgress);
      return `translateY(${translateY}px)`;
    }
    if (animationState === "exiting") {
      const translateX = animation.lockScreenExitTranslateX * animationProgress;
  ...
  const bottomOffset = platform === "ios" ? 120 : 100;
  const iconScale = 0.85;
  const textScale = platform === "ios" ? 0.9 : 0.85;
          ...
          return (
            ...
            >
              {ir.icon ? (
                <div
                  style={{
                    width: icon.size * scale * iconScale,
                    height: icon.size * scale * iconScale,
                    borderRadius: icon.borderRadius * scale * iconScale,
              ...
              >
                ...
                >
                  <span
                    style={{
                      fontSize: text.appName.fontSize * scale * textScale,
...
export default NotificationLockScreen;
