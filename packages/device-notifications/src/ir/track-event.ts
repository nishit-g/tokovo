...
export type NotificationTrackEvent = {
    ...
    _declarationOrder: number;
        ...
        | { type: "NOTIFICATION_DYNAMIC_ISLAND" } & NotificationPayloads["DYNAMIC_ISLAND"]
...
export function isNotificationEvent(event: any): event is NotificationTrackEvent {
    return event.kind === "DEVICE" && typeof event.type === "string" && event.type.startsWith("NOTIFICATION_");
}
