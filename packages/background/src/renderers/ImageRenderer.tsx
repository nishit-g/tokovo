...
import { createScopedLogger, resolveStaticAssetSrc } from "@tokovo/core";
...
export const ImageRenderer: React.FC<ImageRendererProps> = ({ config }) => {
  if (!config.src) {
    log.warn("Image background renderer received no src", {
      event: "background.image.missing_src",
    });
    return null;
  }
  ...
  return (
    <AbsoluteFill style={containerStyle}>
      <Img src={imageSrc} style={imgStyle} pauseWhenLoading />
    </AbsoluteFill>
  );
};
