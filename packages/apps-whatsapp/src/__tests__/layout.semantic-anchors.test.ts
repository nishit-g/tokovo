...
describe("WhatsApp semantic anchors (FEED)", () => {
  ...
  it("settings exposes stable section and row anchors", () => {
  ...
  it("profile distinguishes direct-message and group information anchors", () => {
...
describe("WhatsApp semantic anchors (CHAT)", () => {
  it("projects the same system, reply, reaction, and message IDs as the UI", () => {
    ...
    const snapshot = WhatsAppAnchorProvider.getAnchors(world, layout, "d1");
    expect(snapshot.anchors.m1).toBeTruthy();
    expect(snapshot.anchors.lastMessage).toEqual(snapshot.anchors.m3);
    expect(snapshot.anchors["message-0"]).toBeUndefined();
    expect(snapshot.anchors.message_thread).toBeUndefined();
    expect(snapshot.anchors.inputArea).toBeUndefined();
  });
  ...
  it("exposes authored interaction overlays without DOM geometry", () => {
    ...
    const anchors = WhatsAppAnchorProvider.getAnchors(world, layout, "d1").anchors;
    expect(anchors.reply_composer).toBeTruthy();
    expect(anchors.message_actions).toBeTruthy();
    expect(anchors.media_viewer).toEqual(anchors.device);
    expect(anchors.media_viewer_header).toBeTruthy();
    expect(anchors.media_viewer_content).toBeTruthy();
    expect(anchors.media_viewer_caption).toBeTruthy();
    expect(anchors.status_viewer).toEqual(anchors.device);
    expect(anchors.status_progress).toBeTruthy();
    expect(anchors.status_content).toEqual(anchors.device);
    expect(anchors.status_reply).toBeTruthy();
  });
    ...
    const state = {
      ...
      conversations: {
        room: {
          ...
          name: "Room",
          messages: [
            { id: "m1", from: "ava", type: "text" as const, text: "First", at: 0 },
            { id: "m2", from: "me", type: "text" as const, text: "Last", at: 0 },
          ],
        },
      },
    };
  ...
  it("only publishes anchors for the same bounded window mounted by the UI", () => {
    ...
    const anchored = computeChatLayout(context);
    expect(anchored.semantic?.regions.m20).toBeTruthy();
    expect(anchored.semantic?.regions.m180).toBeUndefined();
    expect(Object.keys(anchored.messageLayouts).length).toBeLessThanOrEqual(120);
  });
});
