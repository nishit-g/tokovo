...
import { WhatsAppConversation, MessageData, WhatsAppGroupMember } from "../types";
...
export const UIStrategyRegistry = {
    ...
    require(id: string): UIStrategy {
        ...
        if (!strategy) {
            throw new Error(`UI Strategy not found: ${id}. Available: ${Array.from(strategies.keys()).join(", ")}`);
        }
        return strategy;
    },
