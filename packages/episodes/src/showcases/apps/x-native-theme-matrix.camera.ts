import { cameraSubject, cinematicProgram } from "@tokovo/dsl";

const LIGHT = "ios_light";
const DIM = "android_dim";
const LIGHTS_OUT = "ios_lights_out_hi";
const group = cameraSubject.group(
  cameraSubject.device(LIGHT, "body"),
  cameraSubject.device(DIM, "body"),
  cameraSubject.device(LIGHTS_OUT, "body"),
);

export const xNativeThemeMatrixCamera = cinematicProgram(
  {
    fps: 30,
    duration: 540,
    stage: {
      width: 1080,
      height: 1920,
      devices: [
        { deviceId: LIGHT, x: 100, y: 245, width: 340, height: 738, zIndex: 10 },
        { deviceId: DIM, x: 640, y: 245, width: 340, height: 742, zIndex: 10 },
        { deviceId: LIGHTS_OUT, x: 370, y: 975, width: 340, height: 738, zIndex: 10 },
      ],
    },
  },
  (cinema) => {
    cinema.plan("theme-matrix", (camera) => {
      camera
        .output("portrait-main", {
          viewport: { x: 0, y: 0, width: 1080, height: 1920 },
          defaultRigId: "matrix",
          coveragePolicy: "require-shots",
          compositionProfileId: "wide-context",
        })
        .rig("matrix", {
          outputId: "portrait-main",
          subject: group,
          composer: {
            screenPosition: [0.5, 0.5],
            targetFill: 0.9,
            fillMode: "contain",
            paddingPx: 48,
            minScale: 0.45,
            maxScale: 1,
          },
          framingGuard: { subject: group, paddingPx: 48, screenPosition: [0.5, 0.5] },
          motion: { type: "minimum-jerk", durationFrames: 24 },
        })
        .shot("native-theme-tableau", "portrait-main", 0, 540, (shot) => {
          shot.target(group).frame({
            screenPosition: [0.5, 0.5],
            targetFill: 0.9,
            fillMode: "contain",
            paddingPx: 48,
            minScale: 0.45,
            maxScale: 1,
          }).settle(24);
        });
    }, { default: true });
  },
);
